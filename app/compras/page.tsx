...
import { ArrowLeftIcon, ShoppingCartIcon, PlusIcon, PencilIcon, TrashIcon } from '@heroicons/react/24/outline';
  ...
  const save = async () => {
    ...
    try {
      ...
      if (modal === 'nuevo') {
        ...
        const res = await fetch(`/api/compras?id=${editing.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            id_compra: form.id_compra,
                ...
                <tbody className="divide-y divide-slate-700/50">
                  {compras.map((c) => (
                    <tr key={c.id} className="hover:bg-slate-800/30">
                      ...
                      <td className="py-3 px-4 text-right font-mono text-amber-400">{formatCurrency(c.subtotal ?? 0)}</td>
                      <td className="py-3 px-4 text-right font-mono text-red-400">{formatCurrency(c.pendiente_mxn ?? 0)}</td>
